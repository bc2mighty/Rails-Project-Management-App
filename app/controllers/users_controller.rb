...
class UsersController < ApplicationController
  ...
  before_action :authorize_login, only: %i[login new]
  before_action :authorize_logout,
                only: %i[dashboard profile projects create_project edit_project show logout destroy threads add_thread]
  def index
    @user = User.new
  end
  ...
  def submit_login
    ...
    if @user&.authenticate(user_params[:password])
      ...
      redirect_to dashboard_users_path,
                  flash: { success: "You are logged in successfully as #{session[:email]}" }
    else
      # render plain: user_params
      flash[:error] = if (user_params[:email] == '') && (user_params[:password] == '')
  ...
  def projects
    ...
    @bgs = ['#68428a', '#a64791', '#855635', '#4c7e8a', '#323e7a', '#943469', '#4d2227', '#b05638', '#4d0b47', '#992b59',
            '#1b5780', '#046e6e', '#254a22', '#6d8748', '#5d6344']
