...
class ProjectsController < ApplicationController
  ...
  def create_user
    ...
    @accesses = %w[write_access read_access update_access delete_access]
    # @accesses.map{|access| project_user[access] == nil ? project_user[access] = false : project_user[access] = true }
    @accesses.each do |access|
      Rails.logger.debug params[:project_user][access]
      project_user[access] = params[:project_user][access] == 'on'
    end
    ...
    @project_user_details = ProjectUser.find_by('user_id = ? and project_id = ?', project_user_params[:user_id],
  ...
  def project_user_params
    params.require(:project_user).permit(:project_user_id, :user_id, :project_id, :write_access, :read_access,
